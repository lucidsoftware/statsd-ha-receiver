...
function Distributor(logger, config) {
	...
	this.replication = Math.min(this.shards.length, this.config.replication || 1);
};
...
Distributor.prototype.assignShards = function(metrics) {
	...
	for (i = 0; i < metrics.length; i++) {
		var shardIndex;
		if (this.replication == this.shards.length) {
			shardIndex = 0;
		}
		else {
		...
		for (var rep = 0; rep < this.replication; rep++) {
...
Distributor.prototype.sendToHost = function(shard, metrics) {
	...
	try {
		if (shard.protocol == "udp4" || shard.protocol == "udp6") {
			...
			var socket = net.createConnection(shard.port, shard.hostname);
			...
			socket.setTimeout(5000, function() {
				if (this.config.debug) {
					this.logger.log("Socket timed out");
				}
				socket.end();
			});
			...
			socket.addListener('error', function(connectionException) {
				if (self.config.debug) {
					self.logger.log(connectionException);
				}
